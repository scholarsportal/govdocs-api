...
async def pytesseract_ocr(request: OCRRequest):
    ...
    key = cache_key("pytesseract", image_path, lang=request.lang, config=request.config)
...
async def got_ocr(request: GOTOCRRequest):
    ...
    try:
        ...
        if request.multi_page and isinstance(request.image_path, list):
            images = [Image.open(get_image_path(path)) for path in request.image_path]
            inputs = processor(images, return_tensors="pt", format=request.format, multi_page=request.multi_page)
        else:
